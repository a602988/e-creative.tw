...
class timthumb {
    ...
    public function __construct(){
        ...
        $this->myHost = preg_replace('/^www\./i', '', $_SERVER['SERVER_NAME']);
        ...
        //轉換可能的中文路徑或檔名
        if (class_exists('Sewii\Filesystem\File')) {
            $this->src = Sewii\Filesystem\File::toLocal($this->src);
        }
        
        //移除特殊網域的絕對 URL
        if (preg_match("/^((ht|f)tp(s|):\/\/)/i", $this->src)) {
            if (isset($GLOBALS['ALLOWED_DOMAINS'])) {
                foreach ($GLOBALS['ALLOWED_DOMAINS'] as $allowedDomain) {
                    if (preg_match('/' . preg_quote($allowedDomain, '/') . '/i', $this->src)) {
                        $this->src = preg_replace ('/^((ht|f)tp(s|):\/\/)([a-zA-Z0-9]([a-zA-Z0-9\-]{0,61}[a-zA-Z0-9])?\.)+[a-zA-Z]{2,6}\//i', '', $this->src);
                        break;
                    }
                }
            }
        ...
        //youtube 支援
        if (preg_match('/https?:\/\/(www\.)?youtube\.com\/.+/', $this->src)) {
            if (preg_match('/[\\?&]v=([^&#]*)/', $this->src, $matches)) {
                $this->src = 'http://img.youtube.com/vi/' . $matches[1] . '/0.jpg';
            }
        }
        //***********************************************************************************************************************//
        ...
        if($this->isURL){
            if(ALLOW_ALL_EXTERNAL_SITES){
                ...
                //***********************************************************************************************************************//
                // 如果外部圖片網址請求是在本機端發出則視為合法
                //***********************************************************************************************************************//
                if (!$allowed) {
                    if (!empty($_SERVER['HTTP_REFERER'])) {
                        $refererInfo = parse_url($_SERVER['HTTP_REFERER']);
                        $refererHost = $refererInfo['host'];
                        $salfHost = preg_replace('/^www\./i', '', $_SERVER["SERVER_NAME"]);
                        if (preg_match('/' . preg_quote($salfHost, '/') . '/i', $refererHost)) {
                            $allowed = true;
                        }
                    }
                }
    ...
    protected function processImageAndWriteToCache($localImage){
        ...
        //***********************************************************************************************************************//
        //新增 $_GET['p'] 參數表示是否使用等比縮圖
        //***********************************************************************************************************************//
        $percent_scale = (bool) $this->param('p', 1);
        //***********************************************************************************************************************//
        
        //***********************************************************************************************************************//
        //停用預設縮圖大小，當無指定時使用原圖大小
        //***********************************************************************************************************************//
        ...
        //***********************************************************************************************************************//
        //如果同時設定 width/height，並且不使用裁減縮圖時
        //***********************************************************************************************************************//
        else if ($new_width && $new_width  && !$zoom_crop) 
        {
            //強制等比例
            if ($percent_scale)
            {
                ...
                if ($width / $height <= $new_width / $new_height) {
                    $percent_width = $new_width;
                    $percent_height = round($new_width * $height / $width);
                    if ($percent_height > $new_height) {
                        $percent = $new_height / $percent_height;
                        $new_width = $percent_width * $percent;
                        $new_height = $percent_height * $percent;
                    }
                }
                //依寬度計算等比
                else {
                    $percent_height = $new_height;
                    $percent_width = round($new_height * $width / $height);
                    if ($percent_width > $new_width) {
                        $percent = $new_width / $percent_width;
                        $new_width = $percent_width * $percent;
                        $new_height = $percent_height * $percent;
                    }
                }
        ...
        if (isset(Configure::$options['watermark'])) 
        {
            $isDoWatermark = true;

            //檢查來源
            $referer = null;
            if (!empty($_GET['HTTP_REFERER'])) $referer = $_GET['HTTP_REFERER'];
            if (!empty($_SERVER['HTTP_REFERER'])) $referer = $_SERVER['HTTP_REFERER'];
            if ($referer && ($refererInfo = parse_url($referer))) {
                $refererHost = $refererInfo['host'];
                $salfHost = preg_replace('/^www\./i', '', $_SERVER["SERVER_NAME"]);
                if (preg_match('/' . preg_quote($salfHost, '/') . '/i', $refererHost)) {
                    if (preg_match('/\?.*action=(manager|uploader).*/i', $referer))
                        $isDoWatermark = false;
                }
            }
            ...
            if ($isDoWatermark) 
            {
                $param = Configure::$options['watermark'];
                if (!empty($param->position) &&
                    ($param->image = Sewii\Template\Media::data($param->image)) &&
                    file_exists($param->image['path'])) 
                {
                    try {
                        $Watermark = new Sewii\Drawing\Watermark();
                        $Watermark->source($tempfile);
                        $Watermark->apply(
                            $param->image['path'],
                            $param->position,
                            $param->size,
                        ...
                        $Watermark->saveAs($tempfile, $param->quality);
                    } catch (Exception $ex) {}
                }
            }
        }
    ...
    protected function getLocalImagePath($src){
        ...
        //***********************************************************************************************************************//
        //修正 $sub_directories 在 windows/unix-like 不同系統下的分解結果
        //***********************************************************************************************************************//
        $sub_directories = preg_split('/\/|\\\/', str_replace($this->docRoot, '', $_SERVER['SCRIPT_FILENAME']));
        //***********************************************************************************************************************//
    ...
    protected function sendImageHeaders($mimeType, $dataSize){
        ...
        if (class_exists('Sewii\Filesystem\File')) {
            $basename = Sewii\Filesystem\Path::decode($basename);
            if (preg_match('/msie/i', $_SERVER['HTTP_USER_AGENT'])) /* for IE */
                $basename = mb_convert_encoding ($basename, 'big5', 'utf8');
        }
